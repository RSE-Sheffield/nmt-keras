...
class estimatorword(QEModel):
    ...
    def build(self):
        ...
        src_embedding = Regularize(src_embedding, params,
                trainable=self.trainable, name='src_embedding')
        ...
        state_below = Regularize(state_below, params,
                trainable=self.trainable, name='state_below')

        state_above = Embedding(params['OUTPUT_VOCABULARY_SIZE'],
                params['TARGET_TEXT_EMBEDDING_SIZE'],
                embeddings_regularizer=l2(params['WEIGHT_DECAY']),
                embeddings_initializer=params['INIT_FUNCTION'],
                trainable=self.trainable,
        ...
        if len(params['INIT_LAYERS']) > 0:
            for n_layer_init in range(len(params['INIT_LAYERS']) - 1):
                ...
                ctx_mean = Regularize(ctx_mean, params,
                        trainable=self.trainable, name='ctx' + str(n_layer_init))
            ...
            initial_state = Regularize(initial_state, params,
                    trainable=self.trainable, name='initial_state')
            ...
            if 'LSTM' in params['DECODER_RNN_TYPE']:
                ...
                initial_memory = Regularize(initial_memory, params,
                        trainable=self.trainable, name='initial_memory')
        ...
        trg_enc_bkw = eval(params['ENCODER_RNN_TYPE'])(
                params['ENCODER_HIDDEN_SIZE'],
                kernel_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                recurrent_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
                bias_regularizer=l2(params['RECURRENT_WEIGHT_DECAY']),
        ...
        enc_qe_bkw = Reverse(
                enc_qe_bkw._keras_shape[2],
                axes=1,
                trainable=self.trainable_est,
                name='reverse_enc_qe_bkw'
                )(enc_qe_bkw)
        ...
        seq_concat = concatenate([enc_qe_frw, enc_qe_bkw],
                trainable=self.trainable_est,
                name='seq_concat'
                )
        ...
        self.model = Model(
                inputs=[src_text, next_words, next_words_bkw, trg_words],
