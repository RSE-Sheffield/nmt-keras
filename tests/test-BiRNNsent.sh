...
conf=tests/config-sent-BiRNN.yml
...
# look in tests/testVals.csv for the correct value to compare deepquest test run against
...
python tests/test.py train -c ${conf} --changes TASK_NAME=$task_name DATASET_NAME=$task_name MODEL_TYPE=$model_type MODEL_NAME=$model_name || true > log-${model_name}-test.txt
