...
if echo $PCC $TESTVAL | awk '{exit ($1-$2)^2<1E-12}'; then
  ...
  python predict.py --dataset datasets/Dataset_${task_name}_srcmt.pkl --model trained_models/${model_name}/epoch_${EPOCH}.h5 --save_path saved_predictions/prediction_${task_name}/ 2>&1 | grep Pearson > predict_pearson.txt
  PCC=$(grep -Po "\d\.\d{4}" predict_pearson.txt)
